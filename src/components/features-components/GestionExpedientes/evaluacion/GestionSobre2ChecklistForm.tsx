...
import {
  GESTION_SOBRE2_CHECKLIST_ITEMS,
  buildSobre2ChecklistPayload,
...
interface GestionSobre2ChecklistFormProps {
  expedienteId: string;
  evaluacionId: string;
}
...
export function GestionSobre2ChecklistForm({
  expedienteId,
  evaluacionId,
}: GestionSobre2ChecklistFormProps) {
  ...
  const allAnswered = Object.keys(respuestas).length >= GESTION_SOBRE2_CHECKLIST_ITEMS.length;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const sobre2 =
          evaluacionData && typeof evaluacionData === "object"
            ? ((evaluacionData as Record<string, unknown>).sobre2 as
                | Record<string, unknown>
                | undefined)
            : undefined;

        const hydrated = hydrateChecklistFromApi(GESTION_SOBRE2_CHECKLIST_ITEMS, sobre2);
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const payload = buildSobre2ChecklistPayload(respuestas, observaciones);
      await evaluarSobre2ChecklistFase3(evaluacionId, payload);

      await generarListaCotejo(expedienteId, evaluacionId)
        .then(() => toast.success("Lista de cotejo generada correctamente."))
        .catch(() => toast.warning("Sobre 2 guardado, pero la lista de cotejo no pudo generarse."));

      const nombre = String(oferente?.nombreProveedorEvaluado ?? "oferente");
      toast.success(`Lista de cotejo de ${nombre} guardada con éxito.`);
      router.push(expedienteTabPath(BASE_PATH, expedienteId, RETURN_TAB));
    } catch (error: unknown) {
      toast.error(error instanceof Error ? error.message : "Error al guardar el Sobre 2");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    return (
      <div className="flex h-screen w-full items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm italic text-muted-foreground">Cargando verificación Sobre 2...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-4xl animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="space-y-4">
          <div className="mb-2 flex items-center gap-3 py-2">
            ...
            <div>
              <h3 className="text-[17px] font-bold leading-tight text-navy">Contenido sobre N°2</h3>
              <p className="text-[13px] italic text-muted-foreground">Oferta técnica y económica</p>
            </div>
          </div>

          <div className="space-y-3">
            {GESTION_SOBRE2_CHECKLIST_ITEMS.map((q) => (
        ...
        <div className="mt-10 flex items-center justify-between border-t border-slate-200 pt-6">
          <Button
            ...
            onClick={() =>
              router.push(evaluacionPath(BASE_PATH, expedienteId, evaluacionId, "sobre-1"))
            }
          >
            Anterior
          </Button>
