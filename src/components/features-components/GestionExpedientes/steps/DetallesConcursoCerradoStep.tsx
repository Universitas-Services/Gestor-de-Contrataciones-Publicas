...
import { CAUSALES_CONCURSO_CERRADO } from "@/lib/modalidades/causalesConcursoCerrado";
import {
  detallesConcursoCerradoSchema,
  type DetallesConcursoCerradoFormValues,
} from "@/lib/schemas/gestionExpedienteSchema";

export interface DetallesConcursoCerradoStepProps {
  initialValues?: DetallesConcursoCerradoFormValues | null;
  onBack: () => void;
  onNext: (data: DetallesConcursoCerradoFormValues) => void;
  readOnly?: boolean;
}

export function DetallesConcursoCerradoStep({
  initialValues = null,
  onBack,
  onNext,
  readOnly = false,
}: DetallesConcursoCerradoStepProps) {
  const form = useForm<DetallesConcursoCerradoFormValues>({
    resolver: zodResolver(detallesConcursoCerradoSchema),
    defaultValues: {
      causalProcedenciaCc: initialValues?.causalProcedenciaCc,
      justificacionCausalCc: initialValues?.justificacionCausalCc ?? "",
      descObjetoContratacionCc: initialValues?.descObjetoContratacionCc ?? "",
      codNomenclaturaProcesoCc: initialValues?.codNomenclaturaProcesoCc ?? "",
    },
  });

  const justificacion = form.watch("justificacionCausalCc") ?? "";
  const descObjeto = form.watch("descObjetoContratacionCc") ?? "";
  const codNomenclatura = form.watch("codNomenclaturaProcesoCc") ?? "";
    ...
    <Form {...form}>
      ...
      >
        <FormField
          ...
          name="causalProcedenciaCc"
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-heading-dark font-bold font-inter text-base">
                Seleccione la causal de procedencia para la modalidad de Concurso Cerrado.
              </FormLabel>
              <p className="text-slate-500 italic text-sm mt-0.5 mb-2 font-inter">
                Artículo 85 LCP; 107.5 RLCP; 37 NORMAS SUNAI.
              </p>
              <FormControl>
                <FormDropdownSelect
                  value={field.value}
                  ...
                  options={CAUSALES_CONCURSO_CERRADO.map((opt) => ({
                    value: opt.value,
                    label: opt.label,
                  }))}
                />
        ...
        <FormField
          control={form.control}
          name="justificacionCausalCc"
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-heading-dark font-bold font-inter text-base">
                Detalle la justificación de la causal seleccionada.
              </FormLabel>
              <FormControl>
                <div className="relative w-full min-w-0 max-w-full">
                  <Textarea
                    ...
                    placeholder="Explique la justificación legal aplicable..."
                  />
