...
import {
  createActoresContratacionDirectaSchema,
  type ActoresContratacionDirectaFormValues,
} from "@/lib/schemas/gestionExpedienteSchema";
import type { TipoContratacionBackend } from "@/lib/schemas/expedienteSchema";
import {
  getUmbralComisionUcau,
  requiereComisionPorUmbral,
} from "@/lib/modalidades/requiereComisionPorUmbral";
...
export interface ActoresContratacionDirectaStepProps {
  tipoContratacion: TipoContratacionBackend;
  valorUcauBase: number;
  initialValues?: ActoresContratacionDirectaFormValues | null;
  onBack: () => void;
  onNext: (data: ActoresContratacionDirectaFormValues) => void;
  readOnly?: boolean;
}

export function ActoresContratacionDirectaStep({
  tipoContratacion,
  valorUcauBase,
  initialValues = null,
  onBack,
  onNext,
  readOnly = false,
}: ActoresContratacionDirectaStepProps) {
  const requiereComision = requiereComisionPorUmbral(tipoContratacion, valorUcauBase);
  const umbralUcau = getUmbralComisionUcau(tipoContratacion);

  const schema = useMemo(
    () => createActoresContratacionDirectaSchema(requiereComision),
    [requiereComision]
  );
  ...
  const form = useForm<ActoresContratacionDirectaFormValues>({
    ...
    defaultValues: {
      ...
      fecEnvioInvitacionCd: initialValues?.fecEnvioInvitacionCd ?? "",
    },
  });
    ...
    <Form {...form}>
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-x-8 gap-y-6 min-w-0 items-stretch">
          ...
          <FormField
            control={form.control}
            name="fecEnvioInvitacionCd"
            render={({ field }) => (
              <FormItem className="min-w-0 flex h-full flex-col gap-0">
                <FormLabel className="text-heading-dark font-semibold font-inter text-sm">
                  Fecha de Solicitud de Oferta / Invitación
                </FormLabel>
                <div className="mt-auto space-y-1 pt-1.5">
                  <Popover>
